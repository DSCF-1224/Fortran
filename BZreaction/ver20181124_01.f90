MODULE ProblemSetting
    ...
    REAL(    KIND=DPR ), PARAMETER :: SpaceStep  = 1.0D+0
    REAL(    KIND=DPR ), PARAMETER :: TimeStep   = 4.0D-1
    
    REAL( KIND=DPR ), DIMENSION(1:3), PARAMETER :: Velocity = (/ 1.0D+0, 1.0D+1, 1.0D+2 /)
    ...
    SUBROUTINE SaveResult( unit, trgt )
	    ...
	    CALL SaveResultUnit( UNIT= UnitNumSave, TRGT= trgt(:,:,3) )
...
MODULE Simulation
    ...
    SUBROUTINE CalcTimeStep( trgt )
        ...
        FORALL( & !
            itr1 = lwb_trgt(1):upb_trgt(1):1, & !
            itr2 = lwb_trgt(2):upb_trgt(2):1  )
            buff(itr1,itr2,1) = laplacian(itr1,itr2,1)*( Velocity(1)*laplacian(itr1,itr2,2) - Velocity(3)*laplacian(itr1,itr2,3) )
            buff(itr1,itr2,2) = laplacian(itr1,itr2,2)*( Velocity(2)*laplacian(itr1,itr2,3) - Velocity(1)*laplacian(itr1,itr2,1) )
            buff(itr1,itr2,3) = laplacian(itr1,itr2,3)*( Velocity(3)*laplacian(itr1,itr2,1) - Velocity(2)*laplacian(itr1,itr2,2) )
        ENDFORALL
